...
**Search the ledger before registering.** Before an item registers hypotheses about an observed behaviour, search the register for rows that already rule on it. Reading the register is not reading the source. D114–D116 spent three items reconstructing mechanisms whose D-numbers were in the source comments the whole time — D66, D72, D74, D75, D91 — and the failure was not conduct but scoping: no rule had ever said to look. (Ruled at D117.)

**A file-and-line citation to unread source is itself a boundary crossing, and must state its provenance at the point of citation** — read scope, ledger row, committed artifact, or disclosed grep; "correct" is not a provenance. A line number is a page, and the page is read, not remembered. Written after the register demonstrated (2026-08-05, the 268-versus-253 registration's adjudication) that a correct citation from an unverified source is indistinguishable, in the record, from a verified one: an attack on a registration cited two engine locations at line precision — one from a grep run during the attack and not disclosed, the other asserted from memory of a ledger row that, on verification, did not contain it — in an attack whose own central claim was that the registration cited something it hadn't checked existed. Both citations were substantively correct; the verifier's audit found that neither had a provenance the record could show, and the one recalled from a row that didn't hold it is the worse of the two, because it is fabrication that happened to be right. Companion to open-the-source-before-citing-it: that practice governs citing a page for a number; this one governs citing the implementation at all.

**The boundary is between a source and its copies, not between fetching and holding.** Written at the practice's first two hard cases (2026-08-05, the arikara and GNIS items, D123/D124). A fetched transcription of a printed work is a relay — a copy that can differ from the artifact behind it — and cannot promote a tier or anchor a byte; the page-level read reaches the artifact. But where a source has NO artifact behind it — a database, a registry, an API — the authoritative endpoint IS the page: a record retrieved from the publisher's own service, with its identifying fields and query date recorded, is the page-level read of that source. The serving-layer clause (D124): a serving layer of the same database by the same publisher is not a copy, it is the database being served; third-party mirrors re-hosting someone else's data remain relays. The test that decides hard cases: a ruled condition that no procedure can satisfy is not a condition — if "open the source" cannot be done by any means under a proposed boundary, the boundary is drawn wrong, not the condition.

### The confidence-field semantic rule (D125)

Filed as its own section, deliberately: the standing practices above are procedures; this is a semantic rule about what a field means, and blurring the two would blur what a practice is. Ruled at D125, from prose the register had already written four times before the rule existed.

1. `confidence` rates source quality; `DISPUTED` records contestation among sources and is not a rung on the quality ladder.
2. The two are not in tension: absence of dispute through absence of alternatives is not agreement — unavailable, not agreed.
3. The gate consequence, stated because it is the live defect: DISPUTED currently gates below MEDIUM, so a well-sourced contested claim is excluded as if weakly sourced. This is a known incorrectness that has FIRED ONCE, at measured cost — #133's pre-upgrade exclusion carried 0.989 pp on every composite — and was corrected only because an instrument ordered for another purpose exposed it. Recorded, not repaired; pin (e) (the D125 guard-WO) watches the boundary so the next firing is caught by CI rather than by accident.

Any ruling crossing the DISPUTED/MEDIUM boundary states which axis moved.
...
**Units are dimensionless points.** Three companies operating under one order occupy an identical position. The battle's valley phase turned historically on an unanchored left flank; a flank that cannot be located cannot be turned. Recorded as the D91 rider; the objective facet closed at D120 ("correctly dimensionless as implemented"); the UNIT-EXTENT half remains open. *Observed in product (2026-08-07, the re-baseline sitting): companies H and K end the day at the identical point, two symbols on one spot, seen by a user and confirmed by probe — the threat is no longer only reasoned about. Still a live constraint on how well the day's geometry can ever be reproduced, now with a visible product cost.*
