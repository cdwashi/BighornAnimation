...
| Bench extent as "D90's described ground" (~2,827 m², the ~60 m neighbourhood read as a sourced area) | **Killed during drafting by static DEM measurement (forty-third, `.claude/bench-terrace-extent.mjs` + `bench-ray-profile.mjs`) — fifteenth design death, third belonging to the adjudicator, and the worst-shaped of the three: the adjudicator cited a source for a value the source does not contain, in the same breath as ruling that extent must be sourced rather than chosen.** D90's "~60 m neighbourhood" is the terrace-search criterion (≤3.5 m relief across a 60 m window) — a search parameter, not a description; Michno p.105 sources existence and location only (and per §6 even that attribution stands at LOW since O5's B3 — the misread happened on top of an already-downgraded chain, "the source says" doubly unavailable). Authorship is two-named: the instrument-carrier (CC) propagated the 2,827 m² through the fortieth and forty-first as the arithmetic anchor without opening D90's text, when the check that killed it was one read; the earlier, cheaper catch was available and missed. What the measurement then showed: absolute-band flood-fill leaks (4.69 km², cap hit, east-bank cells — valley gradient); ray profiles give the terrace ONE physical edge (eastward downhill break 85–100 m, toward river and line — D90's own selection signal) and open shelf on every other bearing, three unbounded. The Bench is the lip of a shelf; any closed extent draws ≥¾ of its boundary by convention; 708 men fit at every candidate density, so capacity-as-area-packing never binds and the 41st's total stacking is behavioural (point-goal convergence), not physical. Mitigation recorded because it is the practice working, not an excuse: the misread died pre-freeze, during drafting, under measure-before-freezing — like the fourteen before it. | Never ruled — killed pre-freeze during drafting, no D-number; reframe (lip/edge, linear) gated on the forty-fourth |
| Generalized ownership scope, "ORD" — any unit with a standing authority (active order or camp-defence commitment) immune to D72 `startPursuit` | **Rejected scope, killed pre-proposal — refuted by measurement before anyone drafted it** (thirty-third measurement, `.claude/ownership-d105-probe.mjs`, 34 completed WO-D105 seeds). The pursuit family vanishes from the north entirely: the wing fight becomes 100% order-axis fire, the hill fight ceases to exist (zero events), total northern killing drops from ~5,200 to ~2,000, coalition killed falls to median 32 — **below its sourced band's low of 36** — and **complete wing destruction goes to 0/34**. Custer's five companies die in every telling of this battle; ORD produces a north where they never do. The steelman raised at review — a unit in contact plausibly *should* respond to what is in front of it, and consensus-initiative warrior bands especially — turned out to be the empirically correct objection: initiative is how the north fights, and deleting it deletes the battle. **The reasoning direction is recorded as a general pattern: precedent justified the principle; measurement selected the scope where the principle holds.** The D92 commitment-hysteresis argument carries the narrow scope (camp-defence holders, "CD") without inheritance stretch; it was never built for ordered units, and the measured north shows why no argument should be sought to extend it. | Declined 07-30 — killed pre-proposal, no D-number; CD proceeds to ruling separately |
| Anticipated fallback clause for camp-defence feature loss (D98 draft) | **Recorded late — 2026-08-02.** Killed pre-freeze by measurement: 0 of 96 committed samples lost their last eligible feature to west-only confinement, so the clause was never needed. **The entry's absence until now is itself a finding:** METHODS §6 has counted this among the four designs killed before freezing since the practice was written, while the register it names as their home never held it — caught by executing the reference, not by reading either document. *Dated pointer, same date:* the D98 row's 2026-08-02 annotation records that camp-defence-without-feature **does** arise in the D108 world by a different cause — 291 switch-to-nothing strandings via `camp-defense.ts:488` — which supersedes the inference drawn from this measurement, not the measurement. The death stands: that clause was for that cause, and for that cause it was still never needed. | Never ruled — killed pre-freeze, no D-number |
...
## 7. The spatial-extent cluster — three representational gaps, one cause

*Recorded 2026-07-29 as a cluster rather than three entries, per adjudication, after measurements twenty-eight through thirty killed three outcome-discriminator designs in succession.*

**The model has no spatial extent, so it cannot surround, cannot be surrounded, and cannot lose horses to anything.** Three gaps surfaced independently and point at the same underlying absence:

| Gap | Surfaced by | What the record supports that the model cannot express |
|---|---|---|
| **Feature extents** | Sixth measurement (frontage derivations killed); O7 (warriors fight cover-conforming); the occupancy reframe | Point features have no ground; bands have no footprint; ~900 warriors stand at one coordinate |
| **Horse attrition** | Twenty-ninth measurement (mounted/afoot discriminator degenerate: 97.7% mounted at overrun, 2:1 destroyed anyway) | Iron Hawk's afoot-discriminator is real — E Company died afoot because its horses were stampeded — but `unit.mounted` is a formation state that resets freely; mounts cannot be killed, stampeded, or lost |
| **Encirclement** | Thirtieth measurement (0 of 826 cavalry-side overrun episodes surrounded at any cut ≤120°; median 4 enemies clustered on one bearing; ~300° open arcs the norm at the moment of overrun) | Keogh's company died in one compact mass, surrounded; the model's pursuers approach along one path and clump — dimensionless points with no envelopment behavior |

This is the **fourth surfacing of the D91 frontage rider** (after D90's uncomputable flank, D96's trigger locality, and PR-5's point-density collapse) — each time in a new guise, each time as the reason a sourced mechanism could not be expressed. The frontage/occupancy bundle has been queued since PR-5 and keeps being validated by things that fail without it; this cluster is the strongest argument yet. The three dead discriminators (escape-availability, mounted/afoot, surrounded-ness — twenty-eighth through thirtieth measurements, preserved probes) are the measured record that the outcome-fork question could not be answered by any state the model currently carries, which is why the D105 resolver's outcome distribution is ruled to *emerge* from movement and catching rather than be decided by a discriminator.
